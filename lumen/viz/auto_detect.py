...
def auto_detect_chart(
    ...
    for col, col_type in zip(columns, column_types, strict=True):
        ...
        if role == "time_dimension":
            ...
            # Try to classify by DB type name
            if col_type.lower() in _NUMERIC_TYPES:
                numeric_cols.append(col)
                measure_cols.append(col)
            else:
                cat_cols.append(col)
...
def _infer_role_from_type(col_type: str) -> str:
    """Infer a role from a canonical DB type name when schema lookup fails."""
    lower = col_type.lower()
    if lower in _TEMPORAL_TYPES:
        return "time_dimension"
    if lower in _NUMERIC_TYPES:
        return "numeric"
    if lower in _TEXT_TYPES:
        return "categorical"
    if lower in _BOOLEAN_TYPES:
        return "categorical"
    return "other"
