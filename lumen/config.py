...
class ConnectionConfig(BaseModel):
    type: str = "postgresql"  # "postgresql" or "duckdb"
    dsn: str = ""  # PostgreSQL DSN
    schema_name: str = "public"  # PostgreSQL schema name
    parquet_path: str = ""  # Local path to Parquet directory (duckdb only)
...
def load_config() -> LumenConfig:
    ...
    Raises on malformed JSON — a system-level error, not a partial failure. Result[T] not needed.
    """
    path = _config_path()
    if not path.exists():
        return LumenConfig()
