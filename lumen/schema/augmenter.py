...
def augment_schema(project_dir: Path, enriched: EnrichedSchema) -> Result[str]:
    """Discover and parse all doc files in project_dir, return merged text.

    Returns Result with data="" and no diagnostics if no docs are found.
    Partial parse failures are reported as warnings in diagnostics.
    """
    result: Result[str] = Result(data="")
    ...
    if yml_path.exists():
        ...
        if not dbt_docs:
            result.warning("AUGMENT_PARSE", f"Failed to parse dbt schema: {yml_path}")
    ...
    if csv_path.exists():
        ...
        if not csv_docs:
            result.warning("AUGMENT_PARSE", f"Failed to parse CSV dictionary: {csv_path}")

    if not dbt_docs and not markdown and not csv_docs:
        return result

    result.data = _format_augmented_docs(dbt_docs, markdown, csv_docs, enriched)
    return result
